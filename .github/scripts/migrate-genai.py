...
prompts = Path("lib/ai/prompts.ts")
text = prompts.read_text()
old_import = 'import { SchemaType, type ResponseSchema } from "@google/generative-ai";'
new_import = 'import { Type, type Schema } from "@google/genai";'
if old_import not in text:
    raise SystemExit("Legacy prompts import not found")
text = text.replace(old_import, new_import, 1)
text = text.replace("ResponseSchema", "Schema")
text = text.replace("SchemaType.", "Type.")
text = text.replace(
    "Hiring decision prediction / readiness recommendation (e.g., 'Strong Hire for Mid-level', 'Borderline — Practice System Design').",
    "Practice-readiness recommendation that identifies the next useful level or focus area without predicting a hiring decision.",
)
prompts.write_text(text)

gemini = Path("lib/ai/gemini.ts")
existing = gemini.read_text()
marker = "function getSimulatedSkills("
if marker not in existing:
    raise SystemExit("Fallback marker not found in gemini.ts")
fallback = marker + existing.split(marker, 1)[1]

head = '''import { GoogleGenAI, type Part } from "@google/genai";
import {
...
gemini.write_text(head + fallback)

env_file = Path(".env.example")
if env_file.exists():
    env_text = env_file.read_text()
    if "GEMINI_TIMEOUT_MS=" not in env_text:
        if env_text and not env_text.endswith("\n"):
            env_text += "\n"
        env_text += "GEMINI_TIMEOUT_MS=20000\n"
